...
from .video_utils import get_video_info_async
...
async def process_video_entry_route(request):
    ...
    try:
        fixed_video_path = zyf_fix_path(video_path)
        # Use the async variant so the ffprobe subprocess does not
        # block the aiohttp event loop while it reads the video header.
        # Previously this called the sync `get_video_info`, which used
        # `subprocess.run` and stalled every other HTTP request for the
        # duration of the ffprobe call (typically 50-3000ms depending
        # on file size).
        frame_rate, total_frames, duration, width, height = await get_video_info_async(fixed_video_path)
